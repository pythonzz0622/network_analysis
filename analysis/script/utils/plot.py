...
def parse_data(df, type=4):
    ...
    if (type == 4) or (type == 5):
        ...
        top5 = temp.groupby(['업체명']).sum()['거래금액'].sort_values(ascending=False).index[:2]
...
class Network_graph():
    ...
    def _get_top5_node_trace(self, G, company):
    ...
    def _get_top5_edge_trace(self, G, weight, type):
        #### make edge
        edge_x = []
        edge_y = []
        annotation_list = []
    ...
    def plotting(self, title, type, fontsize):
        ...
        if (type == 4) or (type == 5):
            ...
            for i in range(len(top5)):
                ...
                # get edge and node
                edge_traces, annotation_list = self._get_top5_edge_trace(G, w, type)
                node_trace = self._get_top5_node_trace(G, top5[i])
