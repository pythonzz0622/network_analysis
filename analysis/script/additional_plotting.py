...
def make_svg(raw: object, title: str):
    # Network_graph 객체를 생성
    obj = plot.Network_graph(raw, show=False)

    # 4가지 유형의 plot을 saving
    obj.plotting(f'{title} 거래처 + 구매처', 1, fontsize=20)
    obj.plotting(f'{title} 판매처', 2, fontsize=20)
    obj.plotting(f'{title} 구매처', 3, fontsize=20)
