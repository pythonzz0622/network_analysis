...
def make_svg(raw, title , fontsize = 20 , only_top5 = True ):
  # Network_graph 객체를 생성
  obj = plot.Network_graph(raw, show=False )
  # 4가지 유형의 plot을 saving
  if only_top5 == False:
    obj.plotting(f'{title} 거래처 + 구매처', 1 , fontsize= fontsize )
    obj.plotting(f'{title} 판매처', 2 , fontsize= fontsize)
    obj.plotting(f'{title} 구매처', 3 , fontsize= fontsize)

  obj.plotting(f'{title} top5 구매처', 4 , fontsize= 20)
  obj.plotting(f'{title} top5 판매처', 5 , fontsize= 20)
